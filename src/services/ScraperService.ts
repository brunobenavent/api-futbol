...
import Team from '../models/Team.js';     // <--- IMPORTANTE
import Season from '../models/Season.js'; // <--- IMPORTANTE
...
export class ScraperService {
  ...
  private async getOrCreateTeam(name: string, slug: string, logo: string | null): Promise<any> {
    ...
    // Si existe pero no tenía logo y ahora sí, actualizamos
  ...
  public async scrapeRound(seasonYear: string, round: number) {
    ...
    console.log(`📡 Scrapeando (Relacional): Temporada ${seasonYear} - Jornada ${round}`);

    // 1. Aseguramos que la Temporada existe
    ...
    try {
      ...
      console.log(`✅ Jornada ${round}: ${matchesData.length} partidos detectados.`);

      for (const m of matchesData) {
        // A. Buscar o Crear Equipos
        ...
        // B. VINCULAR EQUIPOS A LA TEMPORADA (AQUÍ ESTÁ EL CAMBIO)
        // Usamos $addToSet para añadir los IDs al array 'teams' de la temporada SOLO si no están ya.
  ...
  // --- SCRAPEO DETALLE (Sin cambios lógicos, solo funciona igual) ---
  public async scrapeMatchDetail(matchUrl: string) {
    ...
    try {
        ...
        await Match.findOneAndUpdate({ matchUrl: matchUrl }, updateData);
  ...
  public async scrapeFullSeason(season: string) {
    ...
    console.log(`🚀 Iniciando CARGA RELACIONAL de la temporada ${season}...`);
    try {
        for (let i = 1; i <= 38; i++) {
  ...
  // --- CRON (Actualizado para el error que tenías) ---
  public async updateLiveMatches() {
    if (ScraperService.isSeeding) return;
    const now = new Date();
    
    // Necesitamos hacer populate para acceder al año de la temporada
    ...
    }).populate('season'); // <--- IMPORTANTE: Populate
    
    if (liveMatch) {
        // Obtenemos el año de la temporada desde el objeto populado
        const seasonYear = (liveMatch.season as any).year;
        
        console.log(`🔥 EN JUEGO: ID Partido ${liveMatch._id}`);
        await this.scrapeRound(seasonYear, liveMatch.round); // Ahora pasamos string "2026", no ObjectId
